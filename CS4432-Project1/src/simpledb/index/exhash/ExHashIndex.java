...
public class ExHashIndex implements Index {
	...
	public void beforeFirst(Constant searchkey) {
		...
		if (buckets.get(bucket).getTotal() == 4){
			// Increment depths and update buckets
			globalDepth++;
			buckets.get(bucket).incLocalDepth();
			bitMask = (bitMask << 1) + 1;
			addBuckets(); // Add new buckets
	...
	/**
	 * Adds new buckets so that the total is 2^globalDepth
	 */
	public void addBuckets(){
		int i;
		int bucketsToAdd = (globalDepth << 1);
		for (i = 0; i < bucketsToAdd; i++){
			if (buckets.get(i).equals(null)){
				// If the bucket doesn't exist, make a new one
				Bucket newBucket = new Bucket(i, globalDepth);
				buckets.put(i, newBucket);
			}
		}
	}
	 ...
	 */
	public void updateBuckets(int bucket){
